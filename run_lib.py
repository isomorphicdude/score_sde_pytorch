...
def makeGIF(config, workdir, eval_folder="eval", image_dir=None):
    """
    Stores the GIFs produced by the evaluation function in the workdir.
    ...
    # Create directory to image_dir if not none
    if image_dir is not None:
        image_dir = os.path.join(workdir, image_dir)
        tf.io.gfile.makedirs(image_dir)
    else:
        tf.io.gfile.makedirs(os.path.join(workdir, "images"))
    ...
    for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
        ...
        # Initisalise Samplers
        # samplers should return a list of images sampled at each call of update_fn
        if config.eval.enable_sampling:
            sampling_shape = (
                config.eval.batch_size,
                config.data.num_channels,
        ...
        if config.eval.enable_sampling:
            ...
            for r in range(num_sampling_rounds):
                ...
                # A list of samples 
                all_samples, n = sampling_fn(score_model)
                ...
                logging.info("Samples saved to %s" % (image_dir))
